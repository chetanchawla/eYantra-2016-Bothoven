...
/*
  void serialcomm()//Nakul bhaiya ise dekh lena; gets started when data is being received by the fb
  {
    ...
    if(recieve==100)
    {
      Serial.print(curr_pos);//serially sends it's current position to the other bot
      recieve=Serial.read();
      if(recieve==400)
      {
        char sa=Serial.read();
...
{ /*
  ...
  //  whoWillGo();
  //  //After this send slaveDrive to slave
  //    Serial.print(500); //Inform the slave that the master is sending traversal array
  //    delay(500);
  //    Serial.print(indS);//Check this
  //    delay(500);
  //    for(int rasengan=0;rasengan<indS;rasengan++)
  //    {
  //      Serial.print(slaveDrive[rasengan][0]);
  //      delay(500);
  //      Serial.print(slaveDrive[rasengan][1]);
  //      delay(500);
  //    }
  //  Buzzer(500);
  ch = a;
  ...
  for (pos; pos < master_size; pos++)                  //Traverse the MNP array
  {
    ...
    if (reach == master_size - 1)                     //Long Buzzer
      Buzzer(4500);
...
void setup() {
  ...
  UBRR2H = 0x00;
  UCSR2B = 0x98;
  //serial comm initialization over
  whoWillGo();
  //After this send slaveDrive to slave
  chotaBuzzer(2500000);
  delay(500);
