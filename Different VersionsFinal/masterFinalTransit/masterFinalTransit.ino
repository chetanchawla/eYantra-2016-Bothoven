...
Servo myservo1;
Servo myservo2;
int servoLeft = 0, servoRight = 180, pinNo = A11, number, number1, modelNo, DistanceDash; //rotServ=11, strikeServ=12;
float sDistance;
int master_size;
...
{
  Buzzer(500);
  Serial2.println("hit function-next pos");
  ...
  {
    Serial2.println("hi, I'm in same case");
    Serial2.println("number");
    Serial2.println(number);
    ...
    if (obstacle == 1)                                  //obstacle detected
    {
      ...
      else if (its_pos <= 'x' && its_pos >= 'A')
      {
        ...
        if(byakugan==2)  //ye kiya hai change
        {
          ...
          for(int delBlockNote=pos;delBlockNote<indM;delBlockNote++)
          {
            masterDrive[delBlockNote][0]=masterDrive[delBlockNote+1][0];
            masterDrive[delBlockNote][1]=masterDrive[delBlockNote+1][0];
            indM=indM-1;
          }
        }
        /* while(!Serial.available())
