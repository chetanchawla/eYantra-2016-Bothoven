/******************************************************************************************************************************
   ...
   Functions           : calcSizeNe,calcsize,mapp,mapp2,findPath,deterPath,deleteNeighbour,Stop,Forw,Back,Left,Right,face,
                         Line_Follow_Forw,Line_Follow_Back,Front,Count_Right,Count_Left,Forw_Cm,Back_Cm,Buzzer,
...
float sDistance;                                                          /*Distance returned by sharp sensor*/
int master_size;
int mnp[] = {3, 16, 15, 20, 13, 2, 2, 2, 7, 3, 2, 16, 19, 7, 14};//python
//int mnp1[] = {16, 0, 20, 3, 14, 11, 4, 9, 1, 1};
//char mnp[33];//python
int arr[2];
int masterDrive[33][2];                                                    /*array of notes to be traversed by the master*/
int slaveDrive[33][2];                                                     /*array of notes to be traversed by the slave*/
//To be filled according to the table
int notesToMNP[21][2] = {
  {0, 0}, //A6 0
  {0, 0}, //B6  1
...
int Thresh = 40;                                                          /* White Line Sensor Threshhold Value*/
...
int blast;                                                                 /*For obstacle avoidance when slave is at its note- is used as a flag when incorrect data is recieved by serial communication*/
...
int tenWala=10;                                                               /*First Serial Comm handshake bit*/
char its_pos;
/*
  ...
  while (flag == 0)                            /*runs the loop till source(since we are backtracing) is not found*/
  {
    ...
    for (y = 0; y < calcsize(mapp(mml)); y++)   /* used to access all the neighbours of mml*/
    {
      ...
      if (avai == 1)                          /*Adds the neighbour to bws if it does not already exist in it*/
      {
        ...
        {
          flag = 1;
          //Serial2.println("source mil gaya");
          break;
        }
      }
    }
    //Serial2.print("ind: ");
    //Serial2.println(ind);
//    if(ind==47&&bws[ind]!=so)//ye nahin chala
//    {
//      //Serial2.println("Returning 2");
//      return 2;
//      }
//      else
      bwsIndex++;
...
int Line_Follow_Forw()
{
  ...
  if ((ms + rs + ls) > 600)//wannachange 800
  {
...
/*
   ...
   Input              : None
   Output             : None
   Logic              : Called if unreadable data is recieved by xbee. Again recieves the data which it was supposed to recieve through xbee by commanding Slave to send data again, serially writing 1
...
void again_serial()
{
  ...
  else if (its_pos <= 'x' && its_pos >= 'A')
  {
    ...
    delay(100);//hatayi delay 100 to 50
...
//    }
    ...
    delay(20);//hatayi delay 50 to 20
    Serial2.println("Got145");
    //delay(50);
    //Serial2.println("posD");//hataya serial print
    delay(20);
    Serial2.println(posD);
    //delay(50);
    //Serial2.println("valD");//hataya serial print
    delay(20);
    ...
    delay(20);
    Serial2.println(master_size);
    delay(50);
    for(int ras=pos;ras<master_size;ras++)
    {
      if(masterDrive[ras][0]>posD)   //ye change kiya divyansh important
      {
        ...
        //Serial2.println(ras);
        //delay(50);
        for(int temp=(master_size-1);temp>=ras;temp--)
        {
          //Serial2.println("i'm in for");
          //delay(50);
          masterDrive[temp+1][0]=masterDrive[temp][0];
          masterDrive[temp+1][1]=masterDrive[temp][1];
          }
         master_size++;
         masterDrive[ras][0]=posD;
         masterDrive[ras][1]=valD;
         reach = masterDrive[pos][1];
         Rev();
         if(moonfli==2)
         ...
         //delay(50);
         //Serial2.println(masterDrive[ras][0]);
         //delay(50);
         //Serial2.println("sras1");
         //delay(50);
         //Serial2.println(masterDrive[ras][1]);
         //delay(50);
         ...
         //delay(50);
         //for(int inazuma=0;inazuma<master_size;inazuma++)
         //{
          //Serial2.print(masterDrive[inazuma][0]);
          //delay(50);
          //Serial2.print(" ");
          //delay(50);
          //Serial2.println(masterDrive[inazuma][1]);
          //delay(50);
         //}
         allign();
         Play(); 
...
//      Serial.write(145);
//      delay(500);
//      Serial.write(masterDrive[pos][0]);
//      delay(500);
//      Serial.write(masterDrive[pos][1]);
      allign();
      return 2;
      }
      byakugan=Traverse();
...
int Play()
{
  ...
  for (pos; pos < master_size; pos++)                  //Traverse the MNP array
  {
    reach = masterDrive[pos][1];
...
//      Serial.write(1);
      ...
      //Serial.write(masterDrive[pos][1]);
      Serial.write(reach);
      delay(50);//hatayi delay 0 to 50
      //Serial2.println("in 145");//hataya serial print
      //Serial2.println("reach");
      //Serial2.println(reach);
      //Serial2.println("masterDrive[pos][0]");
      //Serial2.println(masterDrive[pos][0]);
      //Serial2.println("masterDrive[pos][1]");
      //Serial2.println(masterDrive[pos][1]);
      ch=Curr_Pos;
      continue;
      }
    while (counter != masterDrive[pos][0])
    {
      if (Serial.available() != 0)
      {
        moonfli=2;
    ...
    Buzzer(500);
    delay(50);
    if (counter == sizeMNP - 1)                 //Long Buzzer
      Buzzer(4500);
    counter = counter + 1;
    delay(50);////hatayi delay 500 to 50
    while(tenWala==10)
...
int Serial_Receive()
{
  ...
  while (1)
  {
    // wait while Serial data is not available
    if (Serial2.available() > 0)
    {
      data = UDR2;
      Buzzer(100);
      // 'f' is the terminanting character
      if (data == 'f')
        return 0;
      // 'k' is the differentiating character
      if (!(data == 'k'))
      {
        while (data == t)
        {
...
void setup() {
  ...
  /*while (!Serial2.available())  //python
  {}
  sizeMNP = Serial2.read();
  //if (sizeMNP == 10)
  //{
    //Buzzer(1000);
  //}
  for (shu = 0; shu < sizeMNP; shu++)
  {
    while (!Serial2.available())
    {}
  ...
  //Buzzer(500);//hata buzzer
 

  whoWillGo();
  //Serial2.println("master ab");//hataya serial print
  //for(int popo=0;popo<indM;popo++)
  //{
  //  delay(50);
  //  Serial2.println(masterDrive[popo][1]);

  //}
  //delay(50);
  //Serial2.println("slave ab");
  //for(int popo=0;popo<indS;popo++)
  //{
  // delay(50);
  //  Serial2.println(slaveDrive[popo][1]);
  //}
  //delay(10000);
  //Buzzer(500);
  //Buzzer(250);
