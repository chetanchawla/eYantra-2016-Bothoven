...
/******************************************************************************************************************************
   ...
   Functions           : calcSizeNe,calcsize,mapp,mapp2,findPath,deterPath,deleteNeighbour,Stop,Forw,Back,Left,Right,face,
                         ...
                         set,allign,Right_D,Left_D,Rev,Play, whoWillGo
...
float sDistance;
//int mnp[] = {16, 0, 20, 3, 14, 11, 4, 9, 1, 1};
...
int Thresh = 40; //wannachange 48                                                         /* White Line Sensor Threshhold Value*/
...
int indS, indM;
int counter = 0;
int same;//used when the firebird has to hit on two mnps from the same node
int flagg;                /*global variable-a dummy value of flag in traverse. Used when wrong data is recieved serially*/
...
/*
  void serialcomm(); //gets started when data is being received by the fb
  {
...
void deterPath(char start, int destNote)
{
  ...
  for (int goku = 0; goku < mnps; goku++)
  {
    ...
    while (ind < siz)
    {
      findPath(start, neigh[destMNP - 1][ind]);
...
void Servo_right(int degg)
{
  for(int i=servoRight;i>degg;i--)
  {
    myservo1.write(i);
    delay(50);
  }
...
void Servo_left(int degg)
{
  for(int i=servoLeft;i<degg;i++)
  {
    myservo1.write(i);
    delay(50);
  }
...
void Servo_middle(int degg)
{
  for(int i=50;i<degg;i++)
  {
    myservo1.write(i);
    delay(50);
  }
...
void hit()
{
  ...
  myservo2.attach(12);
  Back_Cm(3);
  if(Next_Pos=='v'&&Curr_Pos=='w')
  {
    Servo_right(160);
  }
  else if(Next_Pos=='d'&&Curr_Pos=='c')
  {
    Servo_left(5);
  }
  else if(Next_Pos=='U'&&Curr_Pos=='C')
  {
    Servo_left(23);//changed from 40
  }
  else if(Next_Pos=='U'&&Curr_Pos=='X')
  {
    Servo_middle(87);
  }
  else if(Next_Pos=='J'&&Curr_Pos=='K')
  { 
    Servo_middle(90);
  }
  else if(Next_Pos=='j'&&Curr_Pos=='k')
  ...
  else if(Next_Pos=='k'&&Curr_Pos=='J')
  { 
    Servo_left(60);
  }
  else if(Next_Pos=='K'&&Curr_Pos=='k')
  {
    Servo_right(135);
  }
  else if(Next_Pos=='p'&&Curr_Pos=='o')
  {
    Servo_left(0);
  }
  else if(Prev_Pos==Curr_Pos)
...
void setup() {
  ...
  //    Buzzer(1000);
  //    delay(10000);
  //    for (int mayushi = 0; mayushi < shu; mayushi++)
  //    {
  //    Serial2.println(mnp[mayushi]);
  //    }
  //    Buzzer(5000);
